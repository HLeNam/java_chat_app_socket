...
//        updateFileButton();
    ...
    // Thêm phương thức addPopupMenu
    private void addPopupMenu() {
        JPopupMenu popupMenu = new JPopupMenu();

        // Thêm menu mở file
        if (filePath != null && !filePath.isEmpty() && (status.contains("thành công") || status.contains("Đã tải xong"))) {
            JMenuItem openItem = new JMenuItem("Mở file");
            openItem.addActionListener(e -> openFile());
            popupMenu.add(openItem);

            // Thêm separator nếu còn các tùy chọn khác
            popupMenu.addSeparator();
        }

        // Chỉ hiển thị tùy chọn xóa nếu người dùng hiện tại là người gửi tin nhắn và đã thiết lập chatContext
        if (sender.equals(client.getCurrentUser().getUsername()) && chatContext != null) {
            ...
            deleteItem.addActionListener(e -> {
                ...
                if (choice == JOptionPane.YES_OPTION) {
                    client.deleteMessage(fileId, chatContext, isGroup);
                }
            });
            popupMenu.add(deleteItem);
        }

        // Thiết lập menu popup nếu có tùy chọn
    ...
    // Đặt phương thức setChatContext
    public void setChatContext(String chatContext, boolean isGroup) {
        this.chatContext = chatContext;
        this.isGroup = isGroup;

        // Cập nhật lại popup menu
        addPopupMenu();
    }
